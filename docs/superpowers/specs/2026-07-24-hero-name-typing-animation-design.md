...
- Show a green caret while the name is being typed.
- Remove the caret after the animation finishes.
...
Use a CSS-only animation on the existing hero heading. Add a dedicated class to the heading so the effect does not apply to other headings. The typing reveal uses a stepped animation sized to the 14 visible characters in “Lucas Lourenço”; a pseudo-element provides the temporary caret.
...
Extend the portfolio contract test to require the dedicated heading class, stepped one-shot animation, temporary caret, and reduced-motion override. Run the contract tests, lint, TypeScript checking, and the production build.
